...
void Spell::AddScriptedOrSpellFocusTargets(uint32 i, uint32 TargetType, float r, uint32 maxtargets)
{
	for(std::set< Object* >::iterator itr = m_caster->GetInRangeSetBegin(); itr != m_caster->GetInRangeSetEnd(); ++itr)
	{
		...
		if(go->GetInfo()->SpellFocus == m_spellInfo->scr.RequiresSpellFocus)
		{
...
void Spell::AddChainTargets(uint32 i, uint32 TargetType, float r, uint32 maxtargets)
{
	...
	bool RaidOnly = false;
    float range = GetMaxRange(dbcSpellRange.LookupEntry(m_spellInfo->misc.rangeIndex));//this is probably wrong,
...
bool Spell::AddTarget(uint32 i, uint32 TargetType, Object* obj)
{
    ...
    SpellRange* r = dbcSpellRange.LookupEntry(m_spellInfo->misc.rangeIndex);
