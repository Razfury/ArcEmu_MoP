...
Spell::Spell(Object* Caster, SpellEntry* info, bool triggered, Aura* aur)
{
	...
	switch(Caster->GetTypeId())
	{
		...
		default:
			LOG_DEBUG("[DEBUG][SPELL] Incompatible object type, please report this to the devs");
			break;
	}
...
//! Definitely needs work cause I raped the shit out of this function
void Spell::SendSpellStart()
{
    ...
    uint32 castFlags = CAST_FLAG_HAS_TRAJECTORY; // 2
    ...
    //! TODO the other spell for night elves' death (they get another death spell)
    // hacky yeaaaa
    ...
    ObjectGuid casterGuid = p_caster->GetGUID();
...
void Spell::SendSpellGo()
{
    ...
    if (m_missileTravelTime != 0)
        castFlags |= 0x20000; // CAST_FLAG_ADJUST_MISSILE

    if (GetType() == SPELL_DMG_TYPE_RANGED)
        castFlags |= SPELL_GO_FLAGS_RANGED; // 0x20 RANGED

    if (i_caster != NULL)
        castFlags |= SPELL_GO_FLAGS_ITEM_CASTER; // 0x100 ITEM CASTER
    ...
    if (p_caster && p_caster->IsDeathKnight())   //send our rune updates ^^
    {
        ...
        if (cur_have_runes != m_rune_avail_before)
            castFlags |= SPELL_GO_FLAGS_RUNE_UPDATE | SPELL_GO_FLAGS_UNK40000;
    }

    // again, this is not the only death spell! (see SMSG_SPELL_START comment)
    // hacky..
    if (GetProto()->Id == 8326)   // death
        castFlags = SPELL_GO_FLAGS_ITEM_CASTER | 0x0D;
    ...
    ObjectGuid unkGuid = 0;
    bool hasDestLocation = (m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION); // verified
    bool hasDestUnkByte = hasDestLocation; // verified
    bool hasSourceLocation = (m_targets.m_targetMask & TARGET_FLAG_SOURCE_LOCATION); // verified
    bool hasTargetString = m_targets.m_targetMask & TARGET_FLAG_STRING; // verified
    bool hasPredictedHeal = false; // castFlags & CAST_FLAG_HEAL_PREDICTION; //! NOT YET IMPLEMENTED
    bool hasPredictedType = false; // castFlags & CAST_FLAG_HEAL_PREDICTION; //! NOT YET IMPLEMENTED
    ...
    if (hasSourceLocation)
    {
        ObjectGuid srcTransportGuid = m_targets.m_srcTransportGuid;
        data.WriteBit(srcTransportGuid[3]);
        data.WriteBit(srcTransportGuid[7]);
        data.WriteBit(srcTransportGuid[4]);
        data.WriteBit(srcTransportGuid[2]);
    ...
    data.WriteBit(!m_rune_avail_before);
    ...
    data.WriteBit(!hasElevation);
    data.WriteBit(!cur_have_runes); // hasRunesStateAfter
    data.WriteBit(casterGuid[4]);
    ...
    if (hasDestLocation)
    {
        ObjectGuid destTransportGuid = m_targets.m_destTransportGuid;
        data.WriteBit(destTransportGuid[0]);
        data.WriteBit(destTransportGuid[3]);
        data.WriteBit(destTransportGuid[2]);
        data.WriteBit(destTransportGuid[1]);
    ...
    if (hasDestLocation)
    {
        ...
        if (destTransportGuid)
        {
            x = m_targets.m_destPos.m_positionX;
            y = m_targets.m_destPos.m_positionY;
            z = m_targets.m_destPos.m_positionZ;
        }
        //else
          //  m_targets.GetDst()->_position.GetPosition(x, y, z);
        data << z;
    ...
    if (hasSourceLocation)
    {
        ...
        if (srcTransportGuid)
        {
            x = m_targets.m_srcPos.m_positionX;
            y = m_targets.m_srcPos.m_positionY;
            z = m_targets.m_srcPos.m_positionZ;
        }
        //else
        //  m_targets.GetSrc()->_position.GetPosition(x, y, z);
    ...
    if (predictedPowerCount > 0)
    {
        ...
        data << int32(GetProto()->PowerEntry.powerType);
        data << int32(p_caster->GetPowerType());
        //data << int32(p_caster->GetPower(GetProto()->PowerEntry.powerType)); // I think?
        //data << int32(m_caster->GetPower((Powers)m_spellInfo->PowerType));
    }
