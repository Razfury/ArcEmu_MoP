	...
	for(std::list<Pet*>::iterator itr = summons.begin(); itr != summons.end();)
	{
		...
		switch(action)
		{
			...
			case PET_ACTION_SPELL:
				{
					// misc == spellid
					SpellEntry* entry = dbcSpellEntry.LookupEntryForced(misc);
					if(entry == NULL)
						return;
...
void WorldSession::HandlePetSetActionOpcode(WorldPacket & recv_data)
{
	...
	SpellEntry* spe = dbcSpellEntry.LookupEntryForced(spell);
	if(spe == NULL)
		return;
...
void WorldSession::HandlePetSpellAutocast(WorldPacket & recvPacket)
{
	...
	SpellEntry* spe = dbcSpellEntry.LookupEntryForced(spellid);
	if(spe == NULL)
		return;
...
void WorldSession::HandlePetLearnTalent(WorldPacket & recvPacket)
{
	...
	SpellEntry* sp = dbcSpellEntry.LookupEntryForced(te->RankID[ talentcol ]);
	if(sp != NULL)
	{
