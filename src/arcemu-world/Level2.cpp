...
bool ChatHandler::HandleGOInfo(const char* args, WorldSession* m_session)
{
    ...
    SystemMessage(m_session, "%s dynflags:%s%u", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS));
...
bool ChatHandler::HandleGOEnable(const char* args, WorldSession* m_session)
{
    ...
    if (GObj->GetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS) == 1)
	{
		// Deactivate
        GObj->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 0);
		BlueSystemMessage(m_session, "Gameobject deactivated.");
	}
	else
	{
		// /Activate
        GObj->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 1);
