...
bool NoggenFoggerElixr(uint32 i, Spell* pSpell)
{
	...
	{
		case 0:
			pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpellEntry.LookupEntry(16591), true);
			break;
		case 1:
			pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpellEntry.LookupEntry(16593), true);
			break;
		case 2:
			pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpellEntry.LookupEntry(16595), true);
			break;
	}
	return true;
}
...
bool HallowsEndCandy(uint32 i, Spell* pSpell)
{
	...
	SpellEntry* spInfo = dbcSpellEntry.LookupEntryForced(newspell);
...
bool NetOMatic(uint32 i, Spell* pSpell)
{
	...
	SpellEntry* spInfo = dbcSpellEntry.LookupEntryForced(13099);
	if(!spInfo) return true;
...
bool ForemansBlackjack(uint32 i, Spell* pSpell)
{
	...
	SpellEntry* pSpellEntry = dbcSpellEntry.LookupEntry(17743);
...
bool NighInvulnBelt(uint32 i, Spell* pSpell)
{
	...
	if(chance > 10) // Buff - Nigh-Invulnerability - 30456
		pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpellEntry.LookupEntry(30456), true);
	else // Malfunction - Complete Vulnerability - 30457
		pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpellEntry.LookupEntry(30457), true);
...
bool ChampioningTabards(uint32 i, Aura* a, bool apply)
{
	...
	uint32 Faction = a->GetSpellProto()->eff[0].EffectMiscValue;
...
bool DrinkDummyAura(uint32 i, Aura* a, bool apply)
{
	...
	float famount = 2.2f * (a->GetSpellProto()->eff[1].EffectBasePoints / 5);
