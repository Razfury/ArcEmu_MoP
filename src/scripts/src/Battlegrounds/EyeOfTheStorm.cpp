...
void EyeOfTheStorm::OnCreate()
{
	...
	for(i = 0; i < 2; ++i)
	{
		...
		m_bubbles[i]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
		m_bubbles[i]->SetUInt32Value(GAMEOBJECT_FLAGS, 32);
		m_bubbles[i]->SetFaction(114);
		m_bubbles[i]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
...
void EyeOfTheStorm::SpawnBuff(uint32 x)
{
	...
	if(EOTSm_buffs[x] == NULL)
	{
		...
		EOTSm_buffs[x]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
		EOTSm_buffs[x]->SetType(GAMEOBJECT_TYPE_TRAP);
		EOTSm_buffs[x]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
