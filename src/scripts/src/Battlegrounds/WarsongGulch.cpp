...
WarsongGulch::WarsongGulch(MapMgr* mgr, uint32 id, uint32 lgroup, uint32 t) : CBattleground(mgr, id, lgroup, t)
{
	...
	m_homeFlags[0]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
	m_homeFlags[0]->SetType(GAMEOBJECT_TYPE_FLAGSTAND);
	m_homeFlags[0]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);

	// silverwing flag - alliance base
	m_homeFlags[1] = SpawnGameObject(179830, 489, 1540.29f, 1481.34f, 352.64f, 3.17301f, 0, 1314, 2.5f);
	m_homeFlags[1]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
	m_homeFlags[1]->SetType(GAMEOBJECT_TYPE_FLAGSTAND);
	m_homeFlags[1]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
	...
	for(i = 0; i < 2; ++i)
	{
        m_dropFlags[i]->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 1);
...
void WarsongGulch::HookOnAreaTrigger(Player* plr, uint32 id)
{
	...
	if(buffslot >= 0)
	{
		...
		{
			/* apply the buff */
			SpellEntry* sp = dbcSpellEntry.LookupEntry(m_buffs[buffslot]->GetInfo()->sound3);
...
void WarsongGulch::HookFlagDrop(Player* plr, GameObject* obj)
{
	...
	SpellEntry* pSp = dbcSpellEntry.LookupEntry(23333 + (plr->GetTeam() * 2));
...
void WarsongGulch::SpawnBuff(uint32 x)
{
	switch(x)
	{
		case 0:
			...
			m_buffs[x]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
			m_buffs[x]->SetType(GAMEOBJECT_TYPE_TRAP);
			m_buffs[x]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
			break;
		case 1:
...
void WarsongGulch::OnCreate()
{
	...
	gate->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
...
void WarsongGulch::OnStart()
{
	...
	for(list<GameObject*>::iterator itr = m_gates.begin(); itr != m_gates.end(); ++itr)
	{
		...
		(*itr)->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 0);
	}
