...
void AlteracValley::AVNode::Spawn()
{
	...
	else
	{
		...
		if(m_flag == NULL)
		{
			...
			m_flag->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
            m_flag->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 1);
	...
	else
	{
		...
		if(m_aura == NULL)
		{
			...
			m_aura->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
			m_aura->SetUInt32Value(GAMEOBJECT_FLAGS, 1);
			m_aura->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
	...
	else
	{
		...
		if(m_glow == NULL)
		{
			...
			m_glow->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
			m_glow->SetUInt32Value(GAMEOBJECT_FLAGS, 1);
			m_glow->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
	...
	if(!m_template->m_capturable)
	{
		if(m_state != m_template->m_defaultState)
		{
			...
			if(m_flag != NULL)
			{
                ...
                m_flag->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 0);
				m_flag->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
			}
