...
void ArathiBasin::SpawnControlPoint(uint32 Id, uint32 Type)
{
	...
	if(m_controlPoints[Id] == NULL)
	{
		...
		m_controlPoints[Id]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
		m_controlPoints[Id]->SetType(static_cast<uint8>(gi->Type));
		m_controlPoints[Id]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
        m_controlPoints[Id]->SetUInt32Value(OBJECT_FIELD_DYNAMIC_FLAGS, 1);
	...
	if(m_controlPointAuras[Id] == NULL)
	{
		...
		m_controlPointAuras[Id]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 0, 1);
		m_controlPointAuras[Id]->SetType(GAMEOBJECT_TYPE_TRAP);
		m_controlPointAuras[Id]->SetByte(GAMEOBJECT_FIELD_PERCENT_HEALTH, 3, 100);
